...
//    }
//    return cell;
    DTKDropdownMenuCell *cell = [DTKDropdownMenuCell cellWithTableView:tableView];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    DTKDropdownItem *item = self.items[indexPath.row];
     cell.contentLabel.text = item.title;
    if (item.iconName.length) {
        [cell.leftImageView setImage:[UIImage imageNamed:item.iconName]];
    }
    if (self.selectedIndex == indexPath.row&&self.showAccessoryCheckmark) {
        cell.accessoryType = UITableViewCellAccessoryCheckmark;
    }else{
    ...
    cell.contentLabel.textColor = self.textColor;
    if (self.dropDownType == dropDownTypeTitle) {
...
- (UIView *)wrapperView
{
    if (!_wrapperView)
    {
        ...
        _wrapperView.layer.shadowColor = [UIColor grayColor].CGColor;
        _wrapperView.layer.shadowOffset = CGSizeMake(-6, 6);
        _wrapperView.layer.shadowOpacity = 0.5;
        _wrapperView.layer.shadowRadius = 3;
    }
    return _wrapperView;
}
