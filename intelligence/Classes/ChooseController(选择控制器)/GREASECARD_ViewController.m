...
@interface GREASECARD_ViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation GREASECARD_ViewController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    NSDictionary *requestDic;
        ...
        requestDic = @{@"appid":@"GREASECARD",
                       @"objectname":@"GREASECARD",
                       @"curpage":@(page),
                       @"showcount":@(20),
                       @"option":@"read",
                       @"condition":@{@"LICENSENUM":self.LICENSENUM}
                       };
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [GREASECARD mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GREASECARD *one = [self modelTypeWithIndex:indexPath.section];
    cell.top.text = one.CARNUM;
    cell.bottom.text = one.CARDES;
    return cell;
}

- (GREASECARD *)modelTypeWithIndex:(NSInteger)index{
    GREASECARD *model;
    if (!isSearch) {
