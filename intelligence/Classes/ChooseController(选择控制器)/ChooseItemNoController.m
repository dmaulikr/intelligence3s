...
@interface ChooseItemNoController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
    NSInteger _number;
    NSString *_searchStr;
...
@implementation ChooseItemNoController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    NSDictionary *dic = @{@"TESTPRO":@"Y"};
    
    NSDictionary *requestDic = @{@"appid":@"UDPROJECT",
                                 @"objectname":@"UDPRO",
                                 @"curpage":@(page),
                                 @"showcount":@(20),
                                 @"option":@"read",
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [ChooseItemNoModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.chooseItemNo = [self modelTypeWithIndex:indexPath.section];
    [cell.index setText:[NSString stringWithFormat:@"%ld",(long)indexPath.section+1]];

    return cell;
}

- (ChooseItemNoModel *)modelTypeWithIndex:(NSInteger)index{
    ChooseItemNoModel *model;
    if (!isSearch) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!isSearch) {
        ...
        if (self.executeClickCell) {
            self.executeClickCell(model);
        }
    }else{
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    SVHUD_NO_Stop(@"正在加载");
    _number = 1;
    [self searchRequestData:searchBar.text page:_number isUpdata:YES];
