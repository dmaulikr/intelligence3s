...
@interface FlightNoController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation FlightNoController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [FlightNoModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
   // {'appid':'UDFANDETAILS','objectname':'UDFANDETAILS','curpage':1,'showcount':20,'option':'read','sinorsearch':{'LOCNUM':'dd','MODELTYPE':'dd'},'condition':{'PRONUM':'=S1-20140042'}}
    NSDictionary *dic = @{
                          @"LOCNUM":str,
                          @"MODELTYPE":str
                          };
    NSDictionary *dic2 = @{
                           @"PRONUM" : self.requestCoding,
                          };
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.flightNo = [self modelTypeWithIndex:indexPath.section];
    return cell;
}
