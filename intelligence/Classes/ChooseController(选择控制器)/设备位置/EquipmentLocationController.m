...
@interface EquipmentLocationController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation EquipmentLocationController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    // {'appid':'LOCATION','objectname':'LOCATIONS','curpage':1,'showcount':20,'option':'read','condition':{'UDPRONUM':'S1-20140042','UDLOCNUM':'010#'}}
    NSDictionary *dic = @{
                          @"UDPRONUM" : self.requestCoding1,
                          @"UDLOCNUM" : self.requestCoding2,
                          };
    NSDictionary *requestDic = @{@"appid":@"LOCATION",
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [EquipmentLocationModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        
        NSLog(@"查询设备位置  返回 %@",response);
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
    // {'appid':'LOCATION','objectname':'LOCATIONS','curpage':1,'showcount':20,'option':'read','sinorsearch':{'LOCATION':'dd','DESCRIPTION':'dd'},'condition':{'UDPRONUM':'S1-20140042','UDLOCNUM':'010#'}}
    NSDictionary *dic = @{
                          @"LOCATION":str,
                          @"DESCRIPTION":str
                          };
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.equipmentLocation = [self modelTypeWithIndex:indexPath.section];
    return cell;
}

- (EquipmentLocationModel *)modelTypeWithIndex:(NSInteger)index{
    EquipmentLocationModel *model;
    if (!isSearch) {
