//
//  RelatedRepairOrderController.m
//  intelligence
//
...
@interface RelatedRepairOrderController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation RelatedRepairOrderController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *dic = @{@"WORKTYPE":@"FR"};
    
    NSDictionary *requestDic = @{@"appid":@"UDREPORTWO",
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [RelatedRepairOrderModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
...
- (RelatedRepairOrderModel *)modelTypeWithIndex:(NSInteger)index{
    RelatedRepairOrderModel *model;
    if (!isSearch) {
