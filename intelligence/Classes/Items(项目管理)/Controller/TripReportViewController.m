...
@interface TripReportViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation TripReportViewController
...
-(void)rightButtonPress{
    ...
    TripReportAddViewController * add = [[TripReportAddViewController alloc] init];
    add.title = @"新建出差报告";
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    //'appid':'TRIPREPORT','objectname':'UDTRIPREPORT','curpage':1,'showcount':20,'option':'read'
...
-(TripReportModel *)chooseType:(NSInteger)type{
    
    TripReportModel *stock;
    if (self.stcokType == StockTypeNone) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TripReportDetailsViewController *detail=[[TripReportDetailsViewController alloc] init];
    StockViewCell *cell=[tableView cellForRowAtIndexPath:indexPath];
    detail.model = cell.tripReport;
    detail.title = @"出差总结报告详情";
