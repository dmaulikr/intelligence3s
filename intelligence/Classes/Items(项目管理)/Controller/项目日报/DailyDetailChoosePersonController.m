//
//  DailyDetailChoosePersonController.m
//  intelligence
//
...
@interface DailyDetailChoosePersonController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation DailyDetailChoosePersonController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [ChoosePersonModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        
        //        [self.dataArray addObjectsFromArray:array];
        if (self.name.length > 0) {
            for (int i = 0; i < array.count; i++) {
                ChoosePersonModel *model = array[i];
                if (![self.name isEqualToString:model.DEPARTDESC]) {
                    NSLog(@"不同：%@",model.DEPARTDESC);
                }else{
                    NSLog(@"相同：%@ = %@",self.name,model.DEPARTDESC);
...
- (ChoosePersonModel *)modelTypeWithIndex:(NSInteger)index{
    ChoosePersonModel *model;
    if (!isSearch) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!isSearch) {
        ...
        if (self.exetuceClickCell) {
            self.exetuceClickCell(model);
        }
    }else{
