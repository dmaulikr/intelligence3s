...
@interface MaintainRViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation MaintainRViewController
...
-(void)rightButtonPress{
    AddMaintain *add = [[AddMaintain alloc]init];
    add.backName = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"新增维修记录%@%@",dic[@"MAINLOGNUM"],dic[@"success"]];
        HUDNormal(str);
    };
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.maintain = [self chooseType:indexPath.section];
    [cell.index setText:[NSString stringWithFormat:@"%ld",(long)indexPath.section+1]];
    return cell;
}

-(MaintainModel *)chooseType:(NSInteger)type{
    MaintainModel *stock;
    if (self.stcokType == StockTypeNone) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DetailsMaintainViewController *details = [[DetailsMaintainViewController alloc]init];
    details.maintain = stock;
    details.objectname = @"UDCARMAINLOG";
    _number = 1;
    details.BackUpda = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"修改维修记录%@%@",dic[@"CARFUELCHARGENUM"],dic[@"success"]];
        HUDNormal(str);
        if (weakSelf.stcokType == StockTypeNone) {
            [weakSelf requestData:_number isUpdata:YES];
        }else{
