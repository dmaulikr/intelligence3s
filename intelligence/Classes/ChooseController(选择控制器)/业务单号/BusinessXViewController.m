...
@interface BusinessXViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation BusinessXViewController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *requestDic = @{@"appid":@"UDWD",
                                 @"objectname":@"UDTHREEWD",
                                 @"curpage":@(page),
                                 @"showcount":@(20),
                                 @"option":@"read",
                                 @"orderby":@"UDTHREEWDID DESC"};
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [BusinessModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.business = [self modelTypeWithIndex:indexPath.section];
    [cell.index setText:[NSString stringWithFormat:@"%ld",(long)indexPath.section+1]];

    return cell;
}

- (BusinessModel *)modelTypeWithIndex:(NSInteger)index{
    BusinessModel *model;
    if (!isSearch) {
