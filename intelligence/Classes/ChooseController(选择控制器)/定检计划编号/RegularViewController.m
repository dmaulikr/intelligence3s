...
@interface RegularViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *requestDic = @{@"appid":@"REGULARINSPECTIONPLANLINK",
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [RegularModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
    ...
    NSDictionary *requestDic = @{
                                 @"appid":@"REGULARINSPECTIONPLANLINK",
                                 @"objectname":@"REGULARINSPECTIONPLANLINK",
                                 @"curpage":@(page),
                                 @"showcount":@(20),
                                 @"option":@"read",
...
- (RegularModel *)modelTypeWithIndex:(NSInteger)index{
    RegularModel *model;
    if (!isSearch) {
