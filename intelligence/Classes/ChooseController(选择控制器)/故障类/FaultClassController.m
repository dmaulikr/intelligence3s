...
@interface FaultClassController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation FaultClassController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    // {'appid':'FAILURELIST','objectname':'FAILURELIST','curpage':1,'showcount':20,'option':'read','condition':{'TYPE2':'*'}}
    NSDictionary *dic = @{
                          @"TYPE2" : @"*",
                          };
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSMutableArray *array = [FaultClassModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
        for (int i = 0; i<array.count-1; ++i) {
            for (int j=0; j<array.count-i-1; ++j) {
                FaultClassModel *model1 = array[j];
                FaultClassModel *model2 = array[j+1];
                
                if ([model1.FAILURECODE intValue] > [model2.FAILURECODE intValue]) {
                    
                    FaultClassModel* model = array[j];
                    [array replaceObjectAtIndex:j withObject:array[j+1]];
                    [array replaceObjectAtIndex:j+1 withObject:model];
                }
            }
        }
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
    // {'appid':'FAILURELIST','objectname':'FAILURELIST','curpage':1,'showcount':20,'option':'read','sinorsearch':{'FAILURECODE':'dd','CODEDESC':'dd'},'condition':{'TYPE2':'*'}}
    NSDictionary *dic = @{
                          @"FAILURECODE":str,
                          @"CODEDESC":str
                          };
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [FaultClassModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.faultClass = [self modelTypeWithIndex:indexPath.section];
    [cell.index setText:[NSString stringWithFormat:@"%ld",(long)indexPath.section+1]];

    return cell;
}

- (FaultClassModel *)modelTypeWithIndex:(NSInteger)index{
    FaultClassModel *model;
    if (!isSearch) {
        model = self.dataArray[index];
    }else{
        model = self.searchArray[index];
    }
    model.isShow = _isShow;
    return model;
}
