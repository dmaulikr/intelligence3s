...
@interface FinalNumViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation FinalNumViewController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *dic = @{@"APPTYPE" : @"ZYS"};
    NSDictionary *requestDic = @{@"appid":@"UDZYSPLAN",
                                 @"objectname":@"UDINVESTP",
                                 @"curpage":@(page),
                                 @"showcount":@(20),
                                 @"option":@"read",
                                 @"condition":dic};
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [FinalModels mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.final = [self modelTypeWithIndex:indexPath.section];
    return cell;
}

- (FinalModels *)modelTypeWithIndex:(NSInteger)index{
    FinalModels *model;
    if (!isSearch) {
