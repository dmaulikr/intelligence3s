...
@interface FaultCodeController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation FaultCodeController
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
    // {'appid':'FAILURELIST','objectname':'FAILURELIST','curpage':1,'showcount':20,'option':'read','sinorsearch':{'FAILURECODE':'dd','CODEDESC':'dd'},'condition':{'PARENT':'15169'}}
    NSDictionary *dic = @{
                          @"FAILURECODE":str,
                          @"CODEDESC":str
                          };
...
- (FaultCodeModel *)modelTypeWithIndex:(NSInteger)index{
    FaultCodeModel *model;
    if (!isSearch) {
