    ...
    [self checkWFPRequiredWithAppId:@"UDJGWO" objectName:@"WORKORDER" status:self.stock.UDSTATUS compeletion:^(NSArray *fields) {
        ...
        for (NSString *field in fields) {
            if (field.length>0) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"MywindsendAnalysisInfo" object:nil userInfo:@{@"ACTIONCODE":@"WORKORDER",@"ACTIONNAME":@"查看技改工单"}];
}
...
-(void)addThree{
    ...
    self.LT21.operation = ^{
        [weakSelf popInputTextViewContent:weakSelf.LT21.content title:weakSelf.LT21.title compeletion:^(NSString *value) {
            weakSelf.LT21.content=value;
            [weakSelf.tableView reloadData];
        }];
    };
