...
+(URLSessionTask *)baseRequestType:(NSUInteger)type
    ...
    if (type==1) {
        if (params[@"data"]) {
            NSDictionary * dic=[NSDictionary dictionaryWithJSONString:params[@"data"]];
            if ([dic valueForKey:@"appid"]) {
                NSString *appid =[dic valueForKey:@"appid"];
                if ([appid isEqualToString:@"TRIPREPORT"]) {
                    ...
                    if ([dic valueForKey:@"appid"]) {
                        ...
                        if ([appid isEqualToString:@"TRIPREPORT"]) {
                            ...
                            //NSLog(@"JSON:%@",result);
                                ...
                                NSDictionary * dicResult=[NSDictionary dictionaryWithJSONString:result];
