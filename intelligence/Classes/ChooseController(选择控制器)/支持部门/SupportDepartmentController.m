...
@interface SupportDepartmentController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation SupportDepartmentController
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    NSDictionary *requestDic = @{@"appid":@"UDDEPT",
                                 @"objectname":@"UDDEPT",
                                 @"curpage":@(page),
                                 @"showcount":@(20),
                                 @"option":@"read",};
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [SupportDepartmentModel mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.supportDepartment = [self modelTypeWithIndex:indexPath.section];
    return cell;
}

- (SupportDepartmentModel *)modelTypeWithIndex:(NSInteger)index{
    SupportDepartmentModel *model;
    if (!isSearch) {
