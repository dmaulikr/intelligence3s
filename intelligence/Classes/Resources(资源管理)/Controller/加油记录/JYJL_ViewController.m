...
#import "GREASECARD_ViewController.h"

@interface JYJL_ViewController ()

@end

@implementation JYJL_ViewController
...
- (void)viewDidLoad {
    ...
    if(self.Kmodel)
    {
        ...
        [self modifyField:@"录入人" newValue:model.displayName];
        [self modifyField:@"录入人编号" newValue:model.userName];
        
        [self modifyTypeByFieldName:@"DESCRIPTION" newType:@"隐藏"];
        [self modifyTypeByFieldName:@"CARFUELCHARGENUM" newType:@"隐藏"];
    }
}
...
-(void)selectValue:(NSString *)fieldName
{
    ...
    //CARNUM
    if ([fieldName isEqualToString:@"CARNUM"])
    {
        GREASECARD_ViewController * vc = [[GREASECARD_ViewController alloc] init];
        NSString * str =[self valueByname:@"车牌号"];
        if (str.length<=0) {
            HUDNormal(@"请选择车牌号");
        }else{
            vc.LICENSENUM = [self valueByname:@"车牌号"];
            vc.executeCellClick = ^(GREASECARD *model) {
                [self modifyField:@"加油卡编号" newValue:model.CARNUM];
            };
            [self.navigationController pushViewController:vc animated:YES];
        }
    ...
    //NUMBER4
    if ([fieldName isEqualToString:@"NUMBER4"])
    {
        ChoiceWorkView * work;
        work = [[ChoiceWorkView alloc] initWithFrame:self.view.frame dataArray:@[@"0#柴油",@"100#汽油",@"90#汽油",@"92#汽油",@"93#汽油",@"95#汽油",@"97#汽油",@"98#汽油"]];
        [work ShowInView:self.view];
        work.WorkBlock = ^(NSString *str) {
            [self modifyField:@"油品号" newValue:str];
        };
    }
}
//跳转到子表
...
-(void)setDate:(NSString *)name
{
    NSLog(@"设置日期%@",name);
//    CHARGEDATE
    if ([name isEqualToString:@"CHARGEDATE"]) {
        ...
        dateView.dateBlock = ^(NSDate *date) {
            ...
            [self  modifyField:@"加油日期" newValue:dateString];
        };
...
-(void)saveData
{
    ...
    else
    {
        NSArray *arr = @[
                         ...
                         @{@"mboObjectName":@"UDCARFUELCHARGE"},
                         @{@"mboKey":@"CARFUELCHARGENUM"},
                         @{@"mboKeyValue":self.Kmodel.CARFUELCHARGENUM},
                         ];
