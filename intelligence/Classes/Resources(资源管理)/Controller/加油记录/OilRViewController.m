...
@interface OilRViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation OilRViewController
...
-(void)rightButtonPress{
    OilRAddViewController *oil = [[OilRAddViewController alloc]init];
    oil.backName = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"新增加油记录%@%@",dic[@"CARFUELCHARGENUM"],dic[@"success"]];
        HUDNormal(str);
    };
...
-(OilRModel *)chooseType:(NSInteger)type{
    OilRModel *stock;
    if (self.stcokType == StockTypeNone) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DetailsOilViewController *details = [[DetailsOilViewController alloc]init];
    details.stock = stock;
    details.objectname = @"UDCARFUELCHARGE";
    _number = 1;
    details.BackUpda = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"修改加油记录%@%@",dic[@"CARFUELCHARGENUM"],dic[@"success"]];
        HUDNormal(str);
        if (weakSelf.stcokType == StockTypeNone) {
            [weakSelf requestData:_number isUpdata:YES];
        }else{
