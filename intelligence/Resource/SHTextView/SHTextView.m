...
- (void)preSettings
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(endChanged) name:UITextViewTextDidEndEditingNotification object:self];
...
- (void)textChanged
{
    ...
    if ([self.delegate respondsToSelector:@selector(beginChange)]) {
        [self.delegate beginChange];
    }
}
-(void)endChanged{
    [self setNeedsDisplay];
    if ([self.delegate respondsToSelector:@selector(endChange)]) {
        [self.delegate endChange];
    }
}
