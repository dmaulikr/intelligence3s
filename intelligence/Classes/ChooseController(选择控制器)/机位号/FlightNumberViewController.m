...
@interface FlightNumberViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@property (nonatomic,strong) NSMutableArray *numberArrays;
@end
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *dic = @{@"PRONUM" :[NSString stringWithFormat:@"=%@",self.requestCoding]};
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [FlightNoModel mj_objectArrayWithKeyValuesArray:response[@"result"]];
        NSLog(@"array %@",array);
        for (FlightNoModel *model in array) {
            if (![self.numberArrays containsObject:model.MODELTYPE]) {
                [self.numberArrays addObject:model.MODELTYPE];
                [self.dataArray addObject:model];
                NSLog(@"添加了 %@",model.MODELTYPE);
            }
        }
        if (data) {
            // 结束刷新
...
- (void)searchRequestData:(NSString*)str page:(NSInteger)page isUpdata:(BOOL)data{
    ...
    NSDictionary *dic2 = @{
                           @"PRONUM" :[NSString stringWithFormat:@"=%@",self.requestCoding],
                           };
...
//        [self.searchArray addObjectsFromArray:array];
        for (FlightNoModel *model in array) {
            if (![self.numberArray containsObject:model.MODELTYPE]) {
                [self.numberArray addObject:model.MODELTYPE];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    FlightNumberCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [FlightNumberCell flightNumberCell];
    }
    cell.model = [self modelTypeWithIndex:indexPath.section];
    return cell;
}

- (FlightNoModel *)modelTypeWithIndex:(NSInteger)index{
    FlightNoModel *model;
    if (!isSearch) {
