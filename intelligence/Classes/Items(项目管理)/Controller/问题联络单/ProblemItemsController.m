...
@interface ProblemItemsController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation ProblemItemsController
...
//    ProblemDetailsControllerViewController *problem = [[ProblemDetailsControllerViewController alloc]init];
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        if (data) {
            ...
            if(_dataDArray.count < _arrayCount +20){
...
- (void)descending{
    if (self.stcokType == StockTypeNone) {
        NSArray *array = [self.dataDArray sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            NSComparisonResult result = [obj1 compareParkInfo:obj2];
            return result;
        }];
        [self.dataDArray removeAllObjects];
        [self.dataDArray addObjectsFromArray:array];
    }else{
        NSArray *array = [self.searchDArray sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            NSComparisonResult result = [obj1 compareParkInfo:obj2];
            return result;
        }];
        [self.searchDArray removeAllObjects];
        [self.searchDArray addObjectsFromArray:array];
    }
}
...
//                                 @"orderby"   :@"PRORUNLOGNUM desc",
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        if (data) {
            // 结束刷新
            [self.tableview.mj_header endRefreshing];
            if(_searchDArray.count < _arrayCount +20){
                [self.tableview.mj_footer endRefreshingWithNoMoreData];
            }
        }else{
            if(_searchDArray.count < _arrayCount +20){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.problem = [self chooseType:indexPath.section];
    return cell;
}

-(ProblemModel *)chooseType:(NSInteger)type{
    ProblemModel *stock;
    
    if (self.stcokType == StockTypeNone) {
        NSInteger count = self.dataDArray.count - 1;
        stock = self.dataDArray[count - type];
    }else{
        NSInteger count = self.searchDArray.count - 1;
        stock = self.searchDArray[count - type];
    }
    return stock;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.problem = [self chooseType:indexPath.section];
    vc.title = @"联络单详情";
    [self.navigationController pushViewController:vc animated:YES];
}
