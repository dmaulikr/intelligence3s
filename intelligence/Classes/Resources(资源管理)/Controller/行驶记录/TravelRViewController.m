...
@interface TravelRViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation TravelRViewController
...
-(void)rightButtonPress{
    TracelViewController *tracel = [[TracelViewController alloc]init];
    tracel.backModel = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"行驶记录%@新增成功",dic[@"CARDRIVELOGNUM"]];
        HUDNormal(str);
    };
...
-(TravelRModel *)chooseType:(NSInteger)type{
    TravelRModel *stock;
    if (self.stcokType == StockTypeNone) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DetailsTravelViewController *details = [[DetailsTravelViewController alloc]init];
    _number = 1;
    details.BackUpda = ^(NSDictionary *dic){
        NSString *str = [NSString stringWithFormat:@"行驶记录%@修改成功",dic[@"CARDRIVELOGNUM"]];
        HUDNormal(str);
        if (weakSelf.stcokType == StockTypeNone) {
            [weakSelf requestData:_number isUpdata:YES];
        }else{
    ...
    details.stock = stock;
    details.objectname = @"UDCARDRIVELOG";
