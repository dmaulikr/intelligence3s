...
@interface YXJL_ViewController ()

@end

@implementation YXJL_ViewController
...
- (void)viewDidLoad {
    ...
    if (self.Kmodel) {
        ...
        [self queryDisplayNameByUserName:self.Kmodel.PROHEAD FieldName:@"负责人姓名"];
    }else{
        AccountModel *model = [AccountManager account];
        [self modifyField:@"录入时间" newValue:[self.dateAndTimeFormtter stringFromDate:[NSDate date]]];
        [self modifyField:@"录入人" newValue:model.displayName];
        
        [self modifyTypeByFieldName:@"DESCRIPTION" newType:@"隐藏"];
        [self modifyTypeByFieldName:@"LOGNUM" newType:@"隐藏"];
    }
...
- (void)addRightNavBarItem{
    ...
    DTKDropdownItem *item0 = [DTKDropdownItem itemWithTitle:@"工作日志活动"  callBack:^(NSUInteger index, id info) {
...
-(void)selectValue:(NSString *)fieldName
{
    ...
    if ([fieldName isEqualToString:@"PRONUM"]) {
        ...
        choose.executeClickCell = ^(ChooseItemNoModel *model){
            ...
            [self modifyField:@"负责人" newValue:model.RESPONS];
            [self modifyField:@"负责人姓名" newValue:model.RESPONSNAME];
        };
...
-(void)saveData
{
    ...
    else
    {
        NSArray *arr = @[
                         ...
                         @{@"mboObjectName":@"UDRUNLOGR"},
                         @{@"mboKey":@"LOGNUM"},
                         @{@"mboKeyValue":self.Kmodel.LOGNUM},
                         ];
