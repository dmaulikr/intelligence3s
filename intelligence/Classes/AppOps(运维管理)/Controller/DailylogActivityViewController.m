//
//  DailylogActivityViewController.m
//  intelligence
//
...
#
@interface DailylogActivityViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
...
@implementation DailylogActivityViewController
...
-(void)rightButtonPress{
    //    FaultAppAddController *tracel = [[FaultAppAddController alloc]init];
    //    tracel.requestStr = self.requestStr;
    //    [self.navigationController pushViewController:tracel animated:YES];
    
    DailylogActivityAddViewController * add = [[DailylogActivityAddViewController alloc] init];
    add.PRORUNLOGNUM=self.LOGNUM;
    add.title = @"新建工作日志活动列表";
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    NSDictionary *requestDic = @{
                                 @"appid"     :@"UDRUNLINER",
                                 @"objectname":@"UDRUNLINER",
                                 @"curpage"   :@(page),
                                 @"showcount" :@(20),
                                 @"option"    :@"read",
                                 @"orderby"   :@"LOGDATE DESC",
                                 @"condition" :@{@"LOGNUM":self.LOGNUM}
                                 };
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        for (NSDictionary *dic in array) {
            Runliner *stock = [Runliner mj_objectWithKeyValues:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RunlineCell * cell = [tableView dequeueReusableCellWithIdentifier:@"RunlineCell"];
    
    if (!cell) {

        cell = [[[NSBundle mainBundle]loadNibNamed:@"RunlineCell" owner:nil options:nil] lastObject];
    }
    [cell clearText];
    cell.runlineModel=[self chooseType:indexPath.section];
    return cell;
    
}

-(Runliner *)chooseType:(NSInteger)type{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"运行记录详情");
    RunlineCell * cell = [tableView cellForRowAtIndexPath:indexPath];
    Runliner * runlineModel=cell.runlineModel;
    
    DailylogActivityDetailsViewController * details = [[DailylogActivityDetailsViewController alloc] init];
    details.model = runlineModel;
