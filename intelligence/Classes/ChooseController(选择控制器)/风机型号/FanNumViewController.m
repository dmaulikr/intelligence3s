...
@interface FanNumViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    BOOL isSearch;
...
@implementation FanNumViewController
...
- (void)viewDidLoad {
    ...
    if (_type == ChooseTypeFNI) {
        self.title = @"风机型号";
    }else if (_type == ChooseTypeWUZI){
        self.title = @"物资编码";
    }else if (_type == ChooseTypeKUFA){
        self.title = @"库房";
    }
    SVHUD_NO_Stop(@"正在加载");
...
- (void)requestData:(NSInteger)page isUpdata:(BOOL)data{
    ...
    self.task = [HTTPSessionManager getWithUrl:@"/maximo/mobile/common/api" params:dataDic success:^(id response) {
        ...
        NSArray *array = [FanNumModle mj_objectArrayWithKeyValuesArray:response[@"result"][@"resultlist"]];
        NSLog(@"array %@",array);
...
- (FanNumModle *)modelTypeWithIndex:(NSInteger)index{
    FanNumModle *model;
    if (!isSearch) {
